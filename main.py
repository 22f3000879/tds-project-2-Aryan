...
async def run_quiz_process(start_url: str):
    ...
    while current_url and step_count < 15:
        ...
        if task_data.get("file_url"):
            ...
            if not f_url.startswith("http"): f_url = urljoin(current_url, f_url)
            print(f"Downloading file: {f_url}")
            file_summary = parse_file_content(f_url)

        # 4. Solve & Submit (With Retry Loop)
        submit_url = clean_extracted_url(task_data.get("submit_url"))
        if not submit_url: submit_url = "/submit" # Fallback
        if not submit_url.startswith("http"): submit_url = urljoin(current_url, submit_url)
